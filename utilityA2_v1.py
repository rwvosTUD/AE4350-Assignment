...
class Agent:
    ...
    def act(self, state, utils: list):
        ...
        if not self.is_eval:
            ...
            action = choice(range(2), p = actions_prob[0]) 
            
        # define what action == 1 is 
        if action == 1:
            if bool(self.inventory):
                # holding so sell
                action = 2
            else:
                # not holding, thus buy
                action = 1
    ...
    def _reward_type7(self, agent, profit: float, 
        ...
        reward = 0
        prob = at_prob[min(at,1)] #**0.4 # probability of action transformed for better gradient
        if not terminate: 
            '''
    ...
    def handle_action(agent, stats, action, data, t, flags, training = True):
        ...
        if action == 0:
            ...
            if agent.balance > data[t] and len(agent.inventory) == 0: #max one stock 
                ...
                stats.extraCash += extraCash # extra cash required for purchase
